...
# from https://github.com/Flupp/sticky-window-snapping/blob/6bfd702a8fb5d27a1151af27567a7b7afbe47179/tools.bash
# during dev, call ./tools.bash remove install reconfigure
...
function cmd_x_install() {
	local -; set -x
	"kpackagetool${V}" --type=KWin/Script --install .
}
...
function main() {
	...
	declare -g ID
	ID=$(jq --raw-output '.KPlugin.Id' < ./metadata.json)
	while ((${#} > 0))
	do
		cmd "${1}"
		shift
